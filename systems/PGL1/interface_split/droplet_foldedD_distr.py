...
for index, ts in enumerate(u.trajectory[start:end:step]):  
    ...
    for i, frag in enumerate(fragments):
        #NtDD:1-217 CDD:213-449
        foldD_start=i*monomer_length+213
        foldD_end=i*monomer_length+449
...
np.savetxt('droplet_CDD_distr.xvg',dist_result,delimiter='	')

interface_list=[]
core_list=[]
for k in range(33):
    radial_dist=dist_result[:,k+1]
    if radial_dist.mean()>90:
        interface_list.append(k)
    elif radial_dist.mean()<80:
        core_list.append(k)
np.savetxt('interface_CDD_list.xvg',interface_list,delimiter='	')
np.savetxt('core_CDD_list.xvg',core_list,delimiter='	')
...
ax.hlines(y=9,xmin=2000,xmax=30000,color='gray')
#ax.hlines(y=9,xmin=6000,xmax=26000,color='gray')
plt.xlim(2000,30000)
...
plt.savefig('Droplet_CDD_distr.png',dpi=600,bbox_inches='tight')
plt.show()
