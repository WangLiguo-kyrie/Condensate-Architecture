...
for i in range(36):    
    ...
    print(mean(Rg))   
print(mean(Rg_all))  
hist_all,edge_all=np.histogram(Rg_all,bins=np.arange(2,10,0.1))   #define the range 2~10 nm
prob_sll=hist_all/len(Rg_all)

for i in range(36):    
    data = np.loadtxt('Condensate_phase_monomer{}.xvg'.format(i))
    Rg_i = data[:,3]/10
    hist_i,edge_i=np.histogram(Rg_i,bins=np.arange(2,10,0.1))   #define the range 2~10 nm
