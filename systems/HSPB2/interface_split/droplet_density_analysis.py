...
for index, ts in enumerate(u.trajectory[start:end:step]): 
    ...
    #bindwidth=0.3nm
    hist,edge=np.histogram(dist,bins=np.arange(0,180,3))
    hist_collect=hist_collect+hist
frames=len(u.trajectory[start:end:step])
hist_ave=hist_collect/frames

bins=[int((edge[i]+edge[i+1])/2) for i in range(59)]
print(bins)
bins=np.array(bins)
density=[hist_ave[i]/(4*np.pi*bins[i]**2) for i in range(59)]
