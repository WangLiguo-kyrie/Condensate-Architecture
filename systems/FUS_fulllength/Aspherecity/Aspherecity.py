    ...
    if i in Dilute_monomer_list:
        ...
        for index, ts in enumerate(u.trajectory[start:end:step]): 
            if ts.time in dilute_interval[:,0]:
                print('Dilute Protein Monomer {}, Time {}, aspherecity {}'.format(i,ts.time,frag.asphericity()))
                dilute_fragment_result.append([i,ts.time,ts.frame,frag.asphericity()])
            else:
                print('Condensate Protein Monomer {}, Time {}, asphericity {}'.format(i,ts.time,frag.asphericity()))
                condensate_fragment_result.append([i,ts.time,ts.frame,frag.asphericity()])
    else:   
        for index, ts in enumerate(u.trajectory[start:end:step]):    
            print('Condensate Protein Monomer {}, Time {}, asphericity {}'.format(i,ts.time,frag.asphericity()))
            condensate_fragment_result.append([i,ts.time,ts.frame,frag.asphericity()])                 
